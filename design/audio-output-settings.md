...
### 14.2 Settings Launcher 配置（Design Complete — [settings.md](settings.md) §15 正本）

Settings Launcher の **物理配置** は **[settings.md](settings.md) §15 で Design Complete**。**本書では Audio Output Section との接続のみ記載** する。
...
- **App-level Top-right Utility Button**（`Icons.settings_rounded` 相当）
- **Bottom Panel 内・Media Area 内へ配置しない**
- Activate → **`openSettings()`** → Common Settings Shell → Audio → `AudioOutputSettingsSection`
- **Gear → AudioOutputSettingsSection 直結は禁止**

Launcher 未実装 Phase でも **Audio Output Section 単体** の Widget / Controller 実装とテストは可能（Modal / 暫定 route）。

詳細（全 Media 状態でのアクセス、compact landscape、Mobile、Tooltip / Semantics、視覚条件）は [settings.md](settings.md) §15 および [phase2-ui.md](phase2-ui.md) §14。
