...
| 音声出力デバイス | 共通 Model / Service 抽象（Phase A）**実装済み**（[audio-output.md](../design/audio-output.md)）。Settings UI・platform 実装・永続化は **未実装**。設計: システム既定 / 特定デバイス選択、消失時フォールバック、Windows / Android / iOS 各 OS 能力に合わせた UX |
