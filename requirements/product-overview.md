...
| UI 言語 | 次の作業として UI 標準言語を **日本語** へ変更予定。将来、設定画面で言語選択（日本語 / English 等）を実装する。**方式・Flutter localization 構成は未確定** |
| 音声出力デバイス | 設定画面に「オーディオ → 音声出力デバイス」を設ける予定。**現時点では未実装**。初期候補は「システム既定」。利用可能なスピーカー / ヘッドホン / HDMI 等を選択できる設計とする。Audio / Video 共通、選択値の保存・再起動後復元、デバイス消失時はシステム既定へフォールバック。Windows / Android / iOS それぞれの実現方法は調査後に詳細設計。再生中の切替可否は未確定 |
